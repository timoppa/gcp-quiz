const questions = [
  {
    ...
    options: ["A. Configure a new load balancer for the new version of the API", "B. Reconfigure old clients to use a new endpoint for the new API", "Have the old API forward traffic to the new API based on the path", "Use separate backend pools for each API path behind the load balancer"],
